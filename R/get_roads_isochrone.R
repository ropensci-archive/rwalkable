...
#' @param radius The maximum distance to travel along the road network.
...
get_roads_isochrone <- function(nearby_obj, radius, buffer_dist = 0.0002){
  ...
  # Get bbox for extracting roads
  road_bbx <- location_bb

  # Get roads as dodgr graph
  road_net <- get_roads_graph(road_bbx)
  ...
  subs_ind <- is_nearby(goto_mat, location_xy, road_net, radius)
  subs_ind <<- subs_ind[1, ]
